export const metadata = { title: "Terms of Service — JIJI RULING" };

export default function TermsPage() {
  return (
    <main id="main-content" className="privacy-page">
      <article aria-labelledby="terms-title">
        <h1 id="terms-title">TERMS OF SERVICE</h1>
        <p><strong>Effective Date:</strong> January 2026</p>
        <p>By accessing our website, placing an order, or using a service, you agree to these Terms and our <a href="/privacy">Privacy Policy</a>.</p>

        <h2>1. Eligibility</h2>
        <p>You must be at least 18 years old and legally able to enter into a contract to purchase or use our services.</p>

        <h2>2. Our Services</h2>
        <p>JIJI RULING provides digital cultural and personal-reflection services, which may include personalized reports relating to self-understanding, relationships, living spaces, Chinese metaphysics, or similar subjects.</p>
        <p>Service descriptions, prices, estimated delivery times, and included deliverables are displayed on the relevant product page.</p>
        <p>Our services are provided for personal, educational, cultural, and entertainment purposes. They are not medical, psychological, legal, financial, investment, real-estate, or other licensed professional advice.</p>

        <h2>3. No Guaranteed Results</h2>
        <p>Our services involve interpretation and subjective analysis. We do not guarantee:</p>
        <ul>
          <li>The accuracy or completeness of any interpretation;</li>
          <li>A particular personal, relationship, financial, health, or business outcome;</li>
          <li>That future events will occur as described; or</li>
          <li>That following a report will produce a specific result.</li>
        </ul>
        <p>You remain responsible for your own decisions and actions.</p>

        <h2>4. Orders And Payment</h2>
        <p>You agree to provide accurate order, billing, and contact information.</p>
        <p>Prices and available payment methods are shown during checkout. Applicable taxes or currency-conversion charges may be added where required.</p>
        <p>An order is accepted when we send an order confirmation or begin performing the service. We may reject or cancel an order because of suspected fraud, payment failure, legal restrictions, service unavailability, or information that makes the service impractical to complete.</p>
        <p>If we cancel an order before providing the service, we will refund the amount paid.</p>

        <h2>5. Information You Provide</h2>
        <p>You are responsible for providing complete and accurate information needed to perform the service.</p>
        <p>Delays caused by missing, inaccurate, or incomplete information may extend the estimated delivery time.</p>
        <p>If you provide information about another person, you confirm that you have an appropriate lawful basis or permission to provide it. Do not submit information that is unnecessary, unlawful, or obtained without authorization.</p>

        <h2>6. Service Delivery</h2>
        <p>Digital services may be delivered by email, secure download link, customer account, video call, or another method stated on the product page.</p>
        <p>Delivery dates are estimates unless we expressly agree otherwise. We are not responsible for reasonable delays caused by missing customer information, technical failures outside our control, payment reviews, or events beyond our reasonable control.</p>
        <p>You are responsible for providing a working email address and checking spam or junk folders.</p>

        <h2>7. AI Assisted Content</h2>
        <p>We may use artificial intelligence tools to assist with organizing information, translation, drafting, quality review, or preparation of digital reports.</p>
        <p>AI-generated content may contain errors or limitations. Where appropriate, it may be reviewed or edited before delivery. The use of AI does not change the educational, cultural, and non-professional nature of the service.</p>

        <h2>8. Cancellations And Refunds</h2>
        <p>Refund eligibility is governed by our Refund Policy and applicable consumer law.</p>
        <p>Because many services are personalized, refunds may be limited after we begin reviewing your information, preparing your report, conducting a consultation, or delivering digital content.</p>
        <p>Nothing in these Terms removes any cancellation, refund, conformity, or other consumer right that cannot legally be waived.</p>
        <h3>European And UK Consumers</h3>
        <p>If applicable law provides a 14-day withdrawal period, you may exercise that right unless a lawful exception applies.</p>
        <p>If you expressly request that a paid service begin during the withdrawal period, you may be required to pay for the portion already supplied. Where permitted by law, the withdrawal right may end after the service has been fully performed with your prior express request and acknowledgment.</p>
        <p>For immediately supplied digital content, you may be asked to expressly consent to delivery beginning during the withdrawal period and acknowledge any resulting loss of the withdrawal right where permitted by law.</p>

        <h2>9. Personal Use License</h2>
        <p>After full payment, we grant you a limited, non-exclusive, non-transferable license to use your purchased report or other deliverable for personal, non-commercial purposes.</p>
        <p>You may not:</p>
        <ul>
          <li>Resell, sublicense, or commercially distribute a report;</li>
          <li>Publish substantial portions as your own product;</li>
          <li>Remove copyright or ownership notices;</li>
          <li>Use our materials to train an AI model without permission; or</li>
          <li>Copy or exploit our templates, methods, graphics, or content commercially.</li>
        </ul>
        <p>You retain ownership of materials you submit to us. You grant us a limited right to use those materials only as reasonably necessary to provide, secure, and administer the service.</p>

        <h2>10. Intellectual Property</h2>
        <p>The website, branding, text, graphics, templates, service methods, and general materials are owned by or licensed to JIJI RULING and are protected by applicable intellectual-property laws.</p>
        <p>These Terms do not transfer ownership of our intellectual property to you.</p>

        <h2>11. Acceptable Use</h2>
        <p>You may not:</p>
        <ul>
          <li>Use the website for unlawful, fraudulent, or abusive purposes;</li>
          <li>Attempt to access another customer’s account or report;</li>
          <li>Interfere with website security or operation;</li>
          <li>Introduce malware or automated attacks;</li>
          <li>Copy, scrape, or reproduce website content without authorization;</li>
          <li>Submit content that violates another person’s rights; or</li>
          <li>Use our services to harass, exploit, or harm another person.</li>
        </ul>
        <p>We may suspend access or cancel an order when reasonably necessary to prevent misuse, fraud, security risks, or legal violations.</p>

        <h2>12. Third Party Services</h2>
        <p>Our website may use third-party providers for payment processing, hosting, analytics, advertising, email, databases, AI processing, or other functions.</p>
        <p>Their services may be governed by separate terms and privacy policies. We are not responsible for third-party services outside our reasonable control.</p>

        <h2>13. Disclaimer Of Warranties</h2>
        <p>To the extent permitted by law, the website and services are provided on an “as available” basis.</p>
        <p>We do not warrant that the website will always be uninterrupted, error-free, or free from harmful components. We do not exclude warranties or consumer rights that cannot legally be excluded.</p>

        <h2>14. Limitation Of Liability</h2>
        <p>To the extent permitted by law, JIJI RULING will not be liable for indirect, incidental, special, consequential, or punitive damages arising from your use of the website or services.</p>
        <p>Our total liability relating to a particular paid service will not exceed the amount you paid for that service.</p>
        <p>These limitations do not apply where liability cannot legally be limited, including liability for fraud, intentional misconduct, gross negligence, personal injury caused by negligence, or mandatory consumer rights.</p>

        <h2>15. Privacy</h2>
        <p>Our collection and use of personal information are described in our <a href="/privacy">Privacy Policy</a> and Cookie Settings.</p>
        <p>Please review those documents before submitting personal or third-party information.</p>

        <h2>16. Changes To Services Or Terms</h2>
        <p>We may update our website, services, prices, or these Terms.</p>
        <p>Changes will apply from the stated effective date and will not retroactively reduce rights relating to an order already accepted unless required by law.</p>

        <h2>17. Governing Law And Disputes</h2>
        <p>These Terms are governed by the laws of [GOVERNING JURISDICTION], without regard to conflict-of-law principles.</p>
        <p>Before starting formal proceedings, you agree to contact us and attempt to resolve the dispute informally.</p>
        <p>Nothing in this section prevents a consumer from relying on mandatory rights or bringing a claim in a court available under the laws of the consumer’s place of residence.</p>

        <h2>18. General Terms</h2>
        <p>If any provision is found unenforceable, the remaining provisions remain effective.</p>
        <p>Our failure to enforce a provision does not waive our right to enforce it later.</p>
        <p>These Terms, together with the Privacy Policy, Refund Policy, and applicable product description, form the agreement between you and JIJI RULING regarding the purchased service.</p>

        <h2>19. Contact Us</h2>
        <p><strong>JIJI RULING</strong></p>
        <p>Email: <a href="mailto:hello@jijiruling.com">hello@jijiruling.com</a></p>
      </article>
    </main>
  );
}
